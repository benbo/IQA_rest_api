...
def build_index_annoy(h5fname = 'X_ILSVRC2015.hdf5',dset='fc6fc7',out='ILSVRC2015.ann',trees = 128,lazy=True):
    ...
    t.add_item(X.shape[0] - 1, X[-1]) # preallocate full size
...
class ImageSearchAnnoy:
    ...
    '''
    def __init__(self,dimensions,annf='ILSVRC2015.ann',imageListPath = '/home/scratch/benediktb/RegionOfInterest/ILSVRC2015_filelist.txt',prefix = None):
        #load filenames
        with open(imageListPath,'r') as f:
            if not prefix is None:
                self.line_to_file = {i:line.rstrip().replace(prefix,'') for i,line in enumerate(f)}
            else:
